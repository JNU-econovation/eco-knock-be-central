...
class AirQualityQueryService(
    ...
    override fun queryAirQuality(): GetAirQualityResponse {
        ...
        val airQualities = buildList {
            addAll(
                oneMinuteRepository
                    .findBuckets(now.minus(Duration.ofHours(1)), now)
                    .map(AirQualityViewDTO::from)
            )

            addAll(
                fiveMinuteRepository
                    .findBuckets(now.minus(Duration.ofHours(6)), now)
                    .map(AirQualityViewDTO::from)
            )

            addAll(
                fifteenMinuteRepository
                    .findBuckets(now.minus(Duration.ofDays(1)), now)
                    .map(AirQualityViewDTO::from)
            )

            addAll(
                oneHourRepository
                    .findBuckets(now.minus(Duration.ofDays(7)), now)
                    .map(AirQualityViewDTO::from)
            )

            addAll(
                fourHourRepository
                    .findBuckets(now.minus(Duration.ofDays(30)), now)
                    .map(AirQualityViewDTO::from)
            )

            addAll(
                oneDayRepository
                    .findBuckets(now.minus(Duration.ofDays(365)), now)
                    .map(AirQualityViewDTO::from)
            )
        }
    ...
    override fun queryAirQualityTimeseriesHistory(
        ...
        val buckets = findPreviousBuckets(
            resolution = resolution,
            before = before,
            pageable = pageable,
        )
    ...
    private fun findPreviousBuckets(
        ...
        return when (resolution) {
            AirQualityResolution.ONE_MINUTE ->
                oneMinuteRepository.findPreviousBuckets(before, pageable)

            AirQualityResolution.FIVE_MINUTES ->
                fiveMinuteRepository.findPreviousBuckets(before, pageable)

            AirQualityResolution.FIFTEEN_MINUTES ->
                fifteenMinuteRepository.findPreviousBuckets(before, pageable)

            AirQualityResolution.ONE_HOUR ->
                oneHourRepository.findPreviousBuckets(before, pageable)

            AirQualityResolution.FOUR_HOURS ->
                fourHourRepository.findPreviousBuckets(before, pageable)

            AirQualityResolution.ONE_DAY ->
                oneDayRepository.findPreviousBuckets(before, pageable)
        }
    }
}